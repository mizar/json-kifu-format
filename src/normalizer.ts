...
export function canPromote(place: IPlaceFormat, color: Color): boolean {
...
function normalizeMinimalMoves(
  shogi: Shogi,
  moves: IMoveFormat[],
  lastMove?: IMoveFormat
) {
  for (let i = 0; i < moves.length; i++) {
    ...
    if (move.from) {
      ...
      if (
        last &&
        last.move &&
        move.to.x === last.move.to.x &&
        move.to.y === last.move.to.y
      ) {
        move.same = true;
      }
      ...
      if (
        ...
        // 成ってない
        if (
          canPromote(move.to, shogi.turn) ||
          canPromote(move.from, shogi.turn)
        ) {
          move.promote = false;
        }
      }
...
function addRelativeInformation(shogi: Shogi, move: IMoveMoveFormat) {
  ...
  if (moveVectors.length >= 2) {
    const realVector = flipVector(
      shogi.turn,
      spaceshipVector(move.to, move.from)
    );
    move.relative = (() => {
      // 上下方向唯一
      ...
      if (moveVectors.filter((mv) => mv.x === realVector.x).length === 1) {
        if (
          (move.piece === "UM" || move.piece === "RY") &&
          realVector.x === 0
        ) {
          // 直はだめ
          return XToLCR(
            moveVectors.filter((mv) => mv.x < 0).length === 0 ? -1 : 1
          );
        } else {
...
function filterMovesByRelatives(
  relative: string,
  color: Color,
  moves: IMove[]
): IMove[] {
  ...
  for (const move of moves) {
    if (
      relative.split("").every((rel) => moveSatisfiesRelative(rel, color, move))
    ) {
      ret.push(move);
    }
  }
...
function restoreColorOfIllegalAction(moves: IMoveFormat[], shogi: Shogi) {
  if (
    moves.length >= 1 &&
    moves[moves.length - 1].special === "ILLEGAL_ACTION"
  ) {
    moves[moves.length - 1].special =
      (shogi.turn ? "+" : "-") + "ILLEGAL_ACTION";
  }
}
