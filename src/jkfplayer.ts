...
export default class JKFPlayer {
  ...
  public static parse(kifu: string, filename?: string): JKFPlayer {
    if (filename) {
  ...
  public static parseJKF(kifu: string): JKFPlayer {
    JKFPlayer.log("parseJKF", kifu);
    return new JKFPlayer(JSON.parse(kifu));
  }
  public static parseKIF(kifu: string): JKFPlayer {
    JKFPlayer.log("parseKIF", kifu);
    return new JKFPlayer(
      normalizeKIF(parseKIF(JKFPlayer.addLastNewLine(kifu)))
    );
  }
  public static parseKI2(kifu: string): JKFPlayer {
    JKFPlayer.log("parseKI2", kifu);
    return new JKFPlayer(
      normalizeKI2(parseKI2(JKFPlayer.addLastNewLine(kifu)))
    );
  }
  public static parseCSA(kifu: string): JKFPlayer {
    JKFPlayer.log("parseCSA", kifu);
    return new JKFPlayer(
      normalizeCSA(parseCSA(JKFPlayer.addLastNewLine(kifu)))
    );
  }
  public static addLastNewLine(kifu: string): string {
    if (kifu.substr(kifu.length - 1) === "\n") {
      return kifu;
    }
    return kifu + "\n";
  }
  public static numToZen(n: number): string {
    return "０１２３４５６７８９"[n];
  }
  public static numToKan(n: number): string {
    return "〇一二三四五六七八九"[n];
  }
  ...
  public static relativeToKan(relative: string): string {
    return {
      L: "左",
  ...
  public static specialToKan(special: string): string {
    return (
      {
        TORYO: "投了",
  ...
  public static doMove(shogi: Shogi, move: IMoveMoveFormat): Color | undefined {
    if (!move) {
      return;
    }
    if (move.from) {
  ...
  public static undoMove(shogi: Shogi, move: IMoveMoveFormat): void {
    if (!move) {
      return;
    }
    if (move.from) {
  ...
  public initialize(kifu: IJSONKifuFormat): void {
    this.kifu = kifu;
    this.tesuu = 0;
    this.forkPointers = [];
  }
  ...
  public backward(): boolean {
    ...
    this.undoMove(move);
    this.tesuu--;
    this.forkPointers = this.forkPointers.filter(
      (fork) => fork.te <= this.tesuu
    );
    this.updateForksAndCurrentStream();
    return true;
  }
  // tesuu手目へ行く
  public goto(tesuu: number | string): void {
  ...
  // tesuu手前後に移動する
  public go(tesuu: number | string): void {
    if (typeof tesuu === "string") {
      tesuu = Number(tesuu);
    }
    if (isNaN(tesuu)) {
  ...
  public inputMove(move: IMoveMoveFormat): boolean {
  ...
  public getComments(tesuu: number = this.tesuu): string[] {
    return this.getMoveFormat(tesuu).comments || [];
  }
  public getMove(tesuu: number = this.tesuu): IMoveMoveFormat {
  ...
  public getReadableForkKifu(tesuu: number = this.tesuu): string[] {
    return this.getNextFork(tesuu).map((fork) =>
      JKFPlayer.moveToReadableKifu(fork[0])
    );
  }
  public getMaxTesuu(): number {
  ...
  public getState(): IStateFormat {
    return JKFPlayer.getState(this.shogi);
  }
  public getReadableKifuState(): {
    kifu: string;
    forks: string[];
    comments: string[];
  }[] {
  ...
  private updateForksAndCurrentStream() {
    ...
    for (const pointer of this.forkPointers) {
      ...
      currentStream = currentStream.concat(
        currentFork.slice(0, pointer.te - tesuuOffset)
      );
      currentFork =
        currentFork[pointer.te - tesuuOffset].forks[pointer.forkIndex];
